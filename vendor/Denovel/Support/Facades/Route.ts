import { Context, Router, RouterContext } from "./Request.ts";
...
class Routes {
  ...
  async get(
    url: string,
    callback: ((context: RouterContext) => void) | string
  ) {
  ...
  async Controller(path: string) {
    ...
    if (checkFile) {
      ...
      if (typeof controllerMethod === "undefined") {
        return `Controller method name "${methodName}" is not found`;
      }
